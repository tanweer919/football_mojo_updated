...
async function search(query: string): Promise<Hit[]> {
  for (let attempt = 0; attempt <= 4; attempt++) {
    if (attempt > 0) {
      const backoff = attempt === 1 ? 2_000 : 65_000;
      console.warn(`  retry ${attempt}/4 for "${query}" after ${(backoff / 1000).toFixed(0)}s`);
      await sleep(backoff);
    }
    await rateLimit();
    try {
      const res = await axios.get(SPORTSDB_URL, { params: { p: query }, timeout: 20_000 });
...
async function main() {
  console.log(`[refresh-photos-remote] key=${SPORTSDB_KEY === '123' ? '<free-demo>' : '<premium>'} rate=${MAX_PER_MIN}/60s force=${FORCE} dryRun=${DRY_RUN}`);
  ...
  for (const p of allPlayers) {
    ...
    // Skip players that already have a TheSportsDB cutout (unless --force)
    ...
    try {
      // Strategy: try full name first, fall back to surname only
...
async function pushBatch(batch: { playerId: string; photoUrl: string }[]): Promise<number> {
  ...
  try {
    ...
    const { data } = await axios.patch(`${API_BASE}/v1/internal/photos/batch`, { updates: batch }, { timeout: 30_000 });
